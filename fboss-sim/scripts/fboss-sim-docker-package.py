...
def resolve_dependencies(binary_path: Path, installed_dir: Path) -> set[str]:
    ...
    for line in output.splitlines():
        ...
        # A "=> not found" line means the dynamic loader could not resolve a
        # dependency in this environment. Silently dropping it produces an image
        # that builds fine but crash-loops at runtime with "cannot open shared
        # object file", so surface it loudly for the collect-phase logs.
        if lib_path == "not found":
            soname = parts[0].split("(")[0].strip()
            print(
                f"  ⚠ {binary_path.name}: unresolved dependency '{soname}' "
                "(ldd => not found); it will be MISSING from the runtime image"
            )
            continue
...
>>>>>>> 25a6d635e2 (NOS-10708: fboss-sim: package fsdb and the source-built bgpd RPM (#1325))
    ...
    if ret == 0:
        ...
        print(
            f"  • Or manually:   docker run -d --privileged --name fboss_sim_runtime_{USERNAME} {image_tag}"
        )
    else:
