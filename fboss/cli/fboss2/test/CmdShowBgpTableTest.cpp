...
#include "fboss/bgp_policy/if/gen-cpp2/bgp_policy_types.h" // NOLINT(misc-include-cleaner)
...
using namespace facebook::neteng::bgp_policy;
namespace facebook::fboss {
class CmdShowBgpTableTestFixture : public CmdHandlerTestBase {
 public:
...
TEST_F(CmdShowBgpTableTestFixture, queryClient) {
  ...
  auto result = CmdShowBgpTable().queryClient(localhost());
  EXPECT_THRIFT_EQ_VECTOR(*result.tRibEntries(), combinedEntries_);
}

TEST_F(CmdShowBgpTableTestFixture, printOutput) {
  ...
  CmdShowBgpTable().printOutput(tRibEntryWithHost, ss);
...
#ifndef IS_OSS
// Test the CPS annotation (Meta-internal feature)
TEST_F(CmdShowBgpTableTestFixture, printCPSOutput) {
  // PathSelector contains the info for the active criteria
  TPathSelector activeCriteria1;
