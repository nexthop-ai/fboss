...
class ConfigPfcTest : public Fboss2IntegrationTest {
 protected:
  ...
  // config does not crash the HW agent on subsequent warm/cold boots.
  // File-level restore + systemctl restart bypasses fboss2's config rollback,
  // which requires a live thrift connection that may not be available
  // mid-warmboot.
