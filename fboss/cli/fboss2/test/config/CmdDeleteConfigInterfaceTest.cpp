...
//                                                             (SYSTEM_PORT)
// Shared helpers for the whole-object delete fixtures below, which differ only
// in their seed config.
...
// Interface 500 trips both the tunnel and the enabled-member-port check. The
// tunnel is reported, which pins the order: reordering the checks fails this.
...
// the port goes too, so the VLAN is never left with a live port and no
// interface — unlike refusesWhenVlanHasEnabledPort, which names the interface
// alone. Exercises the interfaces-before-ports ordering with both sets filled.
TEST_F(
    CmdDeleteWholeL3InterfaceTestFixture,
