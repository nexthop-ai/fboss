...
#include "fboss/bgp/if/gen-cpp2/bgp_attr_types.h"
...
class NeighborsAdvertisedRejectedTestFixture : public CmdHandlerTestBase {
 public:
  ...
  void SetUp() override {
    ...
    mergedNetwoks_ = advertisedNetworks_;
    mergedNetwoks_.insert(acceptedNetworks_.begin(), acceptedNetworks_.end());
  }
};

class NeighborsAdvertisedRejectedTestFixtureWithoutMed
...
// Test that one route is accepted when two routes are advertised
TEST_F(
    NeighborsAdvertisedRejectedTestFixture,
    queryClientWithTwoAdvertisedRoutes) {
  setupMockedBgpServer();
  EXPECT_CALL(getMockBgp(), getPrefilterAdvertisedNetworks2(_, _))
  ...
  auto results = BgpNeighborsAdvertisedRejected().queryClient(
...
TEST_F(NeighborsAdvertisedRejectedTestFixture, printOutput) {
  setupMockedBgpServer();
  ...
  BgpNeighborsAdvertisedRejected().printOutput(networkPathWithHost, ss);
...
TEST_F(NeighborsAdvertisedRejectedTestFixtureWithoutMed, printOutput) {
  setupMockedBgpServer();
  EXPECT_CALL(getMockBgp(), getRunningConfig(_))
      .WillRepeatedly(Invoke([&](std::string& config) {
        // clang-format off
