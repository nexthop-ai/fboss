...
CmdShowHost::RetType CmdShowHost::createModel(
  ...
  for (const auto& ndpEntry : ndpEntries) {
    ...
    if (ndpEntryPortInfoEntry != portInfoEntries.end()) {
      ...
      hostDetails.inDiscards() = folly::copy(
          ndpEntryPortInfo.input().value().errors().value().discards().value());
      hostDetails.outErrors() = folly::copy(
          ndpEntryPortInfo.output().value().errors().value().errors().value());
  ...
  std::sort(
      ...
      [&](const cli::ShowHostModelEntry& a, const cli::ShowHostModelEntry& b) {
...
void CmdShowHost::printOutput(const RetType& model, std::ostream& out) {
  Table table;
  table.setHeader(
      {"Port",
       "ID",
  ...
  for (const auto& hostEntry : model.hostEntries().value()) {
    table.addRow(
        {hostEntry.portName().value(),
         ...
         folly::to<std::string>(folly::copy(hostEntry.outDiscards().value()))});
  }
  out << table << std::endl;
}
