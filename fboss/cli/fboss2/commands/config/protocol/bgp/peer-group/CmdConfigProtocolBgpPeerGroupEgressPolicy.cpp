...
CmdConfigProtocolBgpPeerGroupEgressPolicyTraits::RetType
CmdConfigProtocolBgpPeerGroupEgressPolicy::queryClient(
  ...
  if (policyMsg.data().empty()) {
    return fmt::format(
        "Error: egress policy name is required for peer-group '{}'",
        peerGroupName);
  }
  std::string policyName = policyMsg.data()[0];

  session.createPeerGroup(peerGroupName);
  session.setPeerGroupEgressPolicy(peerGroupName, policyName);
  session.saveConfig();

  return fmt::format(
      "Successfully set egress policy for peer-group '{}' to: '{}'\n"
      "Config saved to: {}",
      peerGroupName,
      policyName,
...
void CmdConfigProtocolBgpPeerGroupEgressPolicy::printOutput(
