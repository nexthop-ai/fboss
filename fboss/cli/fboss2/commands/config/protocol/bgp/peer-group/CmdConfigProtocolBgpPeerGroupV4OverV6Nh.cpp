...
CmdConfigProtocolBgpPeerGroupV4OverV6NhTraits::RetType
CmdConfigProtocolBgpPeerGroupV4OverV6Nh::queryClient(
  ...
  if (enableMsg.data().empty()) {
    return fmt::format(
        "Error: enable value required for peer-group '{}'", peerGroupName);
  }
  ...
  session.setPeerGroupIpv4OverIpv6Nh(peerGroupName, enable);
  session.saveConfig();

  return fmt::format(
      "Successfully {} v4-over-v6-nexthop for peer-group '{}'\n"
      "Config saved to: {}",
...
void CmdConfigProtocolBgpPeerGroupV4OverV6Nh::printOutput(
