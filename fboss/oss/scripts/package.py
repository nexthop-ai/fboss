...
forwarding_stack = {f"{build_dir}/{bin}": f"bin/{bin}" for bin in [
	"diag_shell_client",
	"fboss2",
...
forwarding_stack_tests = {f"{build_dir}/{bin}": f"bin/{bin}" for bin in [
...
platform_stack.update({
	oss_dir / "hw_sanity_tests/bsp_sanity_tests.conf" : "share/hw_sanity_tests/bsp_sanity_tests.conf",
})

platform_stack_tests = {f"{build_dir}/{bin}": f"bin/{bin}" for bin in [
	"bsp_tests",
