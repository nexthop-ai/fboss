...
# bazel_skylib provides string_flag for //fboss/agent/hw/sai:sai_impl.
# Matches the version other modules already pull in transitively, so the
# root-module pin doesn't conflict with the resolved dependency graph.
_BAZEL_SKYLIB_VERSION = "1.8.2"
# Hedron compile-commands extractor — pinned to a specific commit (not in BCR).
# To upgrade: update the commit hash and run
# `bazel run //fboss/build_defs:refresh_compile_commands`.
...
class BuckTarget:
    ...
    file_path: str = ""
    src: str = ""  # export_file: source path when it differs from name
    # custom_rule fields
...
class BuckParser:
    ...
    def _extract_dict(self, block: str, key: str) -> dict[str, list[str]]:
        ...
        # Match "key": None (no services), e.g. thrift_srcs = {"foo.thrift": None}
        for item_match in re.finditer(r'"([^"]+)"\s*:\s*None\b', dict_content):
            k = item_match.group(1)
            if k not in result:  # don't overwrite list/string matches
                result[k] = []
        return result
    ...
    def _parse_target(
        ...
        return BuckTarget(
            ...
            file_path=file_path,
            src=self._extract_string(block, "src") or "",
...
def generate_module_bazel(deps: dict[str, PrebuiltDep]) -> str:
    ...
    chunks = [
        ...
        # string_flag for //fboss/agent/hw/sai:sai_impl (SAI flavor selection).
        bazel_dep(name = "bazel_skylib", version = "{_BAZEL_SKYLIB_VERSION}")
...
def _discover_getdeps_graph(scratch_path: str) -> list[dict]:
...
def generate_module_bazel_getdeps(
    ...
    sai_impl selects the SAI implementation; "fake" excludes the vendor
    sai_impl dep so no vendor SAI tarball is fetched.
    """
    _indent = "        "  # shared indent for template content
    chunks = [
        f"""\
        ...
        # Run: bazel run //fboss/build_defs:refresh_compile_commands
    ...
    for dep_info in getdeps_graph:
        ...
        # Fake SAI needs no vendor implementation: skip the sai_impl dep so
        # its tarball is never downloaded and @sai_impl is not declared in
        # MODULE.bazel.  libsai's generated BUILD drops its
        # @sai_impl//:sai_impl_headers dep too (_make_build_file_content).
        if sai_impl == "fake" and name == "sai_impl":
            continue
        ...
        build_content = _make_build_file_content(
            dep_obj, bazel_name, inter_deps=inter_deps, sai_impl=sai_impl
        )
        if build_content is None:
            continue
        ...
        if cache_url:
            ...
            if cache_upload:
                oa += f"{attr_indent}cache_upload = True,\n"
...
"""
    ...
    if dep.is_header_only:
        if dep.include_dir == ".":
            ...
            # saiobject.h includes experimental headers as flat names, e.g.:
            #   #include <saiexperimentaldashtrustedvni.h>
            # (no directory prefix — a bare filename on the include path).
            #
            # BCM builds: sai_impl/include/ contains the BCM-specific experimental
            # headers flat in that directory.  Expose only libsai/include/ here
            # so the open-source stubs in libsai/experimental/ don't shadow them.
            # @sai_impl//:sai_impl_headers puts sai_impl/include/ on the path.
            #
            # Fake SAI builds: @sai_impl is absent.  Add libsai/experimental/
            # to the include path (includes = ["include", "experimental"]) so
            # the bare filename resolves to libsai/experimental/<header>.h.
            if sai_impl == "fake":
                hdrs_glob = (
                    'glob(["include/**", "experimental/**"], allow_empty = True)'
                )
                # "experimental" on the include path resolves bare experimental
                # header names (e.g. <saiexperimentaldashtrustedvni.h>) from
                # libsai/experimental/ — the open-source stubs directory.
                includes = '["include", "experimental"]'
                all_deps = [f'"{d}"' for d in (inter_deps or [])]
            else:
                hdrs_glob = 'glob(["include/**"], allow_empty = True)'
                includes = '["include"]'
                all_deps = [f'"{d}"' for d in (inter_deps or [])] + [
                    '"@sai_impl//:sai_impl_headers"'
                ]
            if all_deps:
                extra_deps_str = f"\n    deps = [{', '.join(all_deps)}],"
            else:
...
cc_library(
    ...
    deps = {full_deps},
    # SAI blob non-PIC (-no-pie; mold rejects the abs relocs lld tolerated) + needs C libyaml.
    linkopts = ["-no-pie", "-lyaml"],
    visibility = ["//visibility:public"],
    linkstatic = True,
)
"""
...
class BuildFileGenerator:
    ...
    def __init__(
        ...
        # Files that must be exported (exports_files) from a subpackage because
        # an export_file target in a parent package points its src at them.
        # Maps subpackage rel dir (e.g. "fboss/lib/.../generated_platform_mappings")
        # → set of package-relative file paths.  Applied after all BUILD files
        # are written in generate_all().
        self._pending_exports: dict[str, set[str]] = {}
            ...
            for line in sorted(result.stdout.splitlines()):
                ...
                if rel_dir not in generated_paths:
                    ...
                    if "AUTO-GENERATED" in first_line:
                        build_file.unlink()
                        print(f"  Deleted stale BUILD.bazel: {rel_dir}/BUILD.bazel")
        # Final pass: append exports_files() to subpackages whose files are
        # referenced by export_file targets in parent packages.  Runs after all
        # BUILD files are written so regeneration passes can't drop the exports.
        for subpkg_rel, files in sorted(self._pending_exports.items()):
            build_file = self.repo_root / subpkg_rel / "BUILD.bazel"
            if not build_file.is_file():
                build_file.write_text(
                    "# AUTO-GENERATED by bazelify.py - DO NOT EDIT\n"
                    f"# Source: {subpkg_rel}/BUCK\n\n"
                )
            files_str = ", ".join(f'"{f}"' for f in sorted(files))
            build_file.write_text(
                build_file.read_text()
                + f'\nexports_files([{files_str}], visibility = ["//visibility:public"])\n'
            )
    ...
    def _translate_export_file(self, target: BuckTarget) -> str | None:
        """Translate export_file to exports_files (or filegroup).

        When the target name differs from the source path (export_file with an
        explicit ``src``), a filegroup is emitted so the file is addressable by
        the target name, matching BUCK semantics.
        """
        ...
        rel_dir = target.buck_path.lstrip("/")
        src = target.src or target.name
        file_path = self.repo_root / rel_dir / src
        if not file_path.exists():
            return None
        if src != target.name:
            # If src crosses into a subdirectory that is its own Bazel package
            # (has a BUCK file), reference it with a cross-package label and
            # record it so the subpackage's BUILD gets an exports_files().
            src_label = src
            parts = Path(src).parts
            for i in range(len(parts) - 1, 0, -1):
                subdir = "/".join(parts[:i])
                if (self.repo_root / rel_dir / subdir / "BUCK").is_file():
                    subpkg_rel = f"{rel_dir}/{subdir}"
                    inner = "/".join(parts[i:])
                    self._pending_exports.setdefault(subpkg_rel, set()).add(inner)
                    src_label = f"//{subpkg_rel}:{inner}"
                    break
            lines = ["filegroup("]
            lines.append(f'    name = "{target.name}",')
            lines.append(f'    srcs = ["{src_label}"],')
            lines.append('    visibility = ["//visibility:public"],')
            lines.append(")")
            lines.append("")
            return "\n".join(lines)
...
def _detect_folly_f14_mode(folly_dep: PrebuiltDep) -> int | None:
    ...
    try:
        result = subprocess.run(
            ["nm", str(libfolly)], capture_output=True, text=True, check=False
        )
...
def _compute_input_checksum(repo_root: Path, extra_salt: bytes = b"") -> str:
    ...
    stash/pop, or rebases.  Pass extra_salt to distinguish outputs that differ
    only by a flag (e.g. --sai-impl fake vs bcm) so --if-needed still invalidates.
    """
    hasher = hashlib.sha256()
    if extra_salt:
        hasher.update(extra_salt)
...
""",
    ...
    parser.add_argument(
        "--target", help="Only convert a specific target (e.g., //fboss/agent:core)"
    )
    parser.add_argument(
        "--generate-excludes",
        action="store_true",
    ...
    parser.add_argument(
        "--cache-url", default="", help="HTTP cache URL for dep tarballs"
    )
    parser.add_argument(
        "--cache-upload",
        action="store_true",
        help="Upload freshly built deps to --cache-url (CI only; dev machines "
        "should not publish to the shared cache)",
    )
    parser.add_argument(
        "--if-needed",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default="bcm",
        dest="sai_impl",
        help="SAI implementation to generate for. 'fake' omits the vendor "
        "sai_impl dep from MODULE.bazel and from libsai's generated BUILD "
        "so no vendor SAI tarball is fetched.",
    )
...
def main():  # noqa: PLR0912, PLR0915
    ...
    # Salt with the SAI implementation so switching --sai-impl triggers
    # regeneration even when no input files changed.
    input_checksum = _compute_input_checksum(
        repo_root, extra_salt=args.sai_impl.encode()
    )
    if args.if_needed:
