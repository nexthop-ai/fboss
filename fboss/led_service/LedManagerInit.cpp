...
std::unique_ptr<LedManager> createLedManager() {
  ...
  if (mode == PlatformType::PLATFORM_MONTBLANC) {
    ...
    return std::make_unique<Nova4000LedManager>();
  ...
  } else if (mode == PlatformType::PLATFORM_NH4220F) {
    return std::make_unique<Nh4220fLedManager>();
    ...
    return std::make_unique<Wedge800BNHPLedManager>();
